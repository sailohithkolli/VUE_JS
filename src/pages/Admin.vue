<template>
  <q-page class="bg-grey-3 column">
      ...
      <q-list bordered separator class="bg-white rounded-borders">
        ...
        <q-item v-for="user in ActiveUsernames" :key="user.id" clickable v-ripple>
          <q-item-section avatar>
            <q-avatar>
              <img src="~assets/generic_avatar.png" alt="Avatar" />
            </q-avatar>
          </q-item-section>

          <q-item-section>
            <q-item-label lines="1">{{ user.name }}
              <div>
    ...
    <div class="q-pa-md q-gutter-md">
    <q-list bordered separator class="bg-white rounded-borders">
      <q-item-label header>Inactive Users</q-item-label>
      <q-item v-for="user in InactiveUsernames" :key="user.id" clickable v-ripple>
        <q-item-section avatar>
          <q-avatar>
            <img src="~assets/generic_avatar.png" alt="Inactive Avatar" />
          </q-avatar>
        </q-item-section>
...
export default {
  ...
  methods: {
    ...
    async viewInactiveUsers() {
      try {
        const snapshot = await db.collection('waiting_details').where('active', '==', false).get();
        snapshot.forEach((doc) => {
          const user = doc.data();
          user.id = doc.id;
          this.inactiveUsernames.push(user);
        });
      } catch (error) {
        console.error('Error fetching inactive users:', error);
      }
    },
    async statusToggle(user) {
      // Toggle local record
      user.active = !user.active;
      // Toggle backend db
      await db.collection('waiting_details').doc(user.id).update({ active: user.active });
    },
    async addUser() {
      ...
      await db.collection('waiting_details').add(newuser);
      this.username = '';
      this.phoneNumber = '';
      this.reason = '';
    },
  ...
  computed: {
    ...
    InactiveUsernames() {
      return this.usernames.filter((status) => !status.active);
    },
  },
  created() {
    this.initialize();
    this.viewInactiveUsers();
  },
};
</script>
